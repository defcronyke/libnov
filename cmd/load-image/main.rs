...
const QUAD: [Vertex; 6] = [
	Vertex { a_Pos: [ -1.28, 1.28 ], a_Uv: [0.0, 1.0] },
    Vertex { a_Pos: [  1.28, 1.28 ], a_Uv: [1.0, 1.0] },
    Vertex { a_Pos: [  1.28,-1.28 ], a_Uv: [1.0, 0.0] },
    Vertex { a_Pos: [ -1.28, 1.28 ], a_Uv: [0.0, 1.0] },
    Vertex { a_Pos: [  1.28,-1.28 ], a_Uv: [1.0, 0.0] },
    ...
    B: hal::Backend,
{
    fn new(
        ...
        unsafe {
            ...
            cmd_buffer.copy_buffer_to_image(
                ...
                iter::once(command::BufferImageCopy {
                    buffer_offset: 0,
                    // buffer_width: dims.width / (image_stride as u32),
        ...
        let pipeline = {
            let vs_module = {
                let spirv = auxil::read_spirv(Cursor::new(
                    &include_bytes!("../../shader/load-image.vert.spv")[..],
                ))
                .unwrap();
            ...
            let fs_module = {
                let spirv = auxil::read_spirv(Cursor::new(
                    &include_bytes!("../../shader/load-image.frag.spv")[..],
                ))
                .unwrap();
                unsafe { device.create_shader_module(&spirv) }.unwrap()
            };
